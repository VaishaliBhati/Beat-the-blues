  ...
  var animationTime = options.animationTime || 200 // how long it takes to animate chat bubble, also set in CSS
  var typeSpeed = options.typeSpeed || 5 // delay per character, to simulate the machine "typing"
  ...
  var recallInteractions = options.recallInteractions || 0 // number of interactions to be remembered and brought back upon restart
  var inputCallbackFn = options.inputCallbackFn || false // should we display an input field?
  var responseCallbackFn = options.responseCallbackFn || false // is there a callback function for when a user clicks on a bubble button
  ...
  let interactionsSave = function(say, reply) {
  ...
  let interactionsSaveCommit = function() {
    if (!localStorageAvailable) return
  ...
  this.answer = function(key, content) {
    ...
    // let blob =  await text_to_speech(_convo[key].says[0]);
...
function prepHTML(options) {
  ...
  var relative_path = options.relative_path || "/static/"
  ...
  appendCSS(relative_path + "input.css")
  appendCSS(relative_path + "reply.css")
  appendCSS(relative_path + "says.css")
  appendCSS(relative_path + "setup.css")
  appendCSS(relative_path + "typing.css")
}
